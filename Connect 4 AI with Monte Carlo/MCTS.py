...
https://github.com/Somewheref/Connect4_MCTS/blob/master/MCTS.py
UCT
c * sqrt((2 * ln(N(v))) / (N(v')))
where:
- v': current node
...
class UCT(object):
    ...
    def evaluate_actions(self, actions):
        ...
        actions_stats = [(a, self.stats[self.board.next_state(a, self.history[-1], player=self.current_player)]) for a
                         in actions]
